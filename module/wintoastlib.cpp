...
namespace Util {
    ...
    /* Zroya: INT64 notificationID parameter added */
    inline HRESULT setEventHandlers(_In_ IToastNotification* notification, _In_ std::shared_ptr<IWinToastHandler> eventHandler, _In_ INT64 expirationTime, INT64 notificationID) {
        ...
        /* Zroya: Added notificationID parameter to lambda's capture clausule. */
                    ...
                    if (SUCCEEDED(hr)) {
                        ...
                        if (SUCCEEDED(hr)) {
                            ...
                            if (arguments && *arguments) {
                                /* Zroya: Added notificationID to call */
                    ...
                    if (SUCCEEDED(hr)) {
                        ...
                        if (SUCCEEDED(hr)) {
                            ...
                            if (SUCCEEDED(hr)) {
                                ...
                                /* Zroya: GUID generating move up here. Event handler now knows the notification ID */
                                GUID guid;
                                hr = CoCreateGuid(&guid);

                                if (SUCCEEDED(hr)) {
                                    /* Zroya: Added GUID (NotificationID) to call */
                                    hr = Util::setEventHandlers(notification.Get(), std::shared_ptr<IWinToastHandler>(handler), expiration, guid.Data1);
                                    if (FAILED(hr)) {
                                        setError(error, WinToastError::InvalidHandler);
                                    }
                                }
